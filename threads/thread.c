...
#define MIN_WAKEUP_TICK 0
uint64_t min_wakeup_tick;
...
thread_init (void) {
	...
	min_wakeup_tick = MIN_WAKEUP_TICK;
}
...
thread_get_load_avg (void) {
	...
	old_level = intr_disable ();
	lavg = fp_to_int(mult_mixed(load_avg, 100));
	intr_set_level (old_level);
	return lavg;
}
...
thread_get_recent_cpu (void) {
	...
	old_level = intr_disable ();
	recent_cpu = fp_to_int(mult_mixed(thread_current()->recent_cpu, 100));
	intr_set_level (old_level);
	return recent_cpu;
}
...
void thread_sleep(uint64_t ticks) {
	...
	if (list_empty(&sleep_list) || list_entry(list_back(&sleep_list), struct thread, elem)->wakeup_tick < ticks) {
...
void thread_wakeup(uint64_t ticks) {	
	...
	if (list_empty(&sleep_list) || ticks < get_min_wakeup_tick())
		return;
...
void update_min_wakeup_tick(void) {
...
uint64_t get_min_wakeup_tick(void) {
...
void mlfqs_update_load_avg (void) {
	int ready_threads;
	ready_threads = (thread_current() == idle_thread) ? list_size(&ready_list) :
